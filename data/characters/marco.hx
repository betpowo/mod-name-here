...
function initAnimations() {
	var fps:Float = 18;
	playerSpr.loadGraphic(Paths.image('characters/marco'), true, 50, 50);
...
function update(elapsed) {
	if (toAdd) {
		...
		playerSpr.setPosition(self.x + self.globalOffset.x + (self.width * 0.5), self.y + self.globalOffset.y);
		playerSpr.y = -500;
		groundLevel = self.y + self.height + self.globalOffset.y - 30;
		initialX = playerSpr.x;
	}
...
function onHitDown() {
	if (!states.jump) {
		states.crouch = true;
		crouchTime = Conductor.stepCrochet / 666;
...
function onHitUp() {
	if (!states.jump) {
		states.jump = true;
		playerSpr.velocity.y = -1000 * (states.crouch ? (2 / 3) : 1);
		playerSpr.y -= 3;
		if (!states.crouch)
...
function updateState(elapsed) {
	...
	if (idling) {
		...
		if (idleTime >= 0.6) {
			...
			if (playerSpr.x == initialX && playerSpr.animation.name != 'idle') {
				playerSpr.animation.play('idle', true);
				playerSpr.flipX = false;
			}
		}
