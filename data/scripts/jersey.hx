...
var kick = FlxG.sound.load(Paths.sound('jersey/kick'), 0.5);
var squeak1 = FlxG.sound.load(Paths.sound('jersey/squeak1'), 0.75);
var squeak2 = FlxG.sound.load(Paths.sound('jersey/squeak2'), 0.75);
if (doVocals) {
...
function stepHit(curStep) {
	...
	var totalSteps = (Conductor.stepsPerBeat * Conductor.beatsPerMeasure);
	var index = Math.floor(curMeasureFloat * totalSteps) % totalSteps;
	if (kickSteps.indexOf(index) != -1) {
		kick.play(true);
		inst.volume = 0.0;
	}
}
...
function onNoteHit(e) {
	if (!doVocals || e.note.strumLine.data.type > 1)
		return;
