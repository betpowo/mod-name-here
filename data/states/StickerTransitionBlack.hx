...
function create(e) {
	...
	if (e.transOut) {
		var buh = new FunkinSprite();
		buh.makeSolid(FlxG.width, FlxG.height, 0xffffffff);
		buh.blend = BlendMode.SUBTRACT;
		buh.alpha = 0;
		add(buh);
		FlxTween.tween(buh, {alpha: 1}, 0.2, {startDelay: 0.8});
	}
